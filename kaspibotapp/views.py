        ...
        if request.method == 'POST':
            ...
            for span in soup.find_all('span', text=re.compile('₹')):
                ...
                if '-' in spantext or 'filter' in spantext or 'Under' in spantext or 'Over' in spantext or 'Up' in spantext or 'delivery' in spantext:
                    ...
                    plink = BeautifulSoup(parenttag).find(
                        'a')['href'].replace("/url?q=", "")
                    pprice = spantext
                    pfrom = span.parent.text.split("from ")[-1]

                    print("\nProduct: {}\nLink: {}\nPrice: {}\nFrom: {}\n".format(
                        pname, plink, pprice, pfrom))
        else:
            context = {}
...
def testing(request):
    ...
    for span in soup.find_all('span', text=re.compile('₹')):
        ...
        if '–' in spantext or 'filter' in spantext or 'Under' in spantext or 'Over' in spantext or 'Up' in spantext or 'delivery' in spantext:
            ...
            plink = BeautifulSoup(parenttag, 'html.parser').find('a')[
                'href'].replace("/url?q=", "")
            pprice = spantext
            pfrom = span.parent.text.split("from ")[-1]
...
def dash(request):
    if request.user.is_authenticated:
        if request.method == "POST":
            ...
            url_to_scrape = "https://www.google.com/search?q={}&sca_esv=571272750&rlz=1C1CHBD_enIN925IN925&tbm=shop&sxsrf=AM9HkKk0G6R_h-7c9KG-IpuVpgBCJ6IENg:1696589103326&source=lnms&sa=X&ved=2ahUKEwijltX2nuGBAxXnzzgGHTDJDtoQ_AUoAXoECAUQAw&biw=1536&bih=786&dpr=1.25".format(
                searchterm)
            response = requests.get(url_to_scrape)
            html_document = response.text
            soup = BeautifulSoup(html_document, 'html.parser')
